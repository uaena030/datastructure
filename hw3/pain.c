...
tree *CBT(int front, int rear, int *route, int **Nodemem, tree *curNode, int Time)
{
    ...
    if (front + 1 != rear)
    {
        ...
        { // insert Lchild
            ...
            // deal with capacity
            if (Nodemem[Time][route[front]]-- < 0 || Nodemem[Time][route[mid]]-- < 0 || Time - 1 < 0)
            { // reach edge
                deleteTree(curNode);
                return 0;
            }
        ...
        { // insert Rchild
            ...
            // deal with capacity
            if (Nodemem[TT][route[mid]]-- < 0 || Nodemem[TT][route[rear]]-- < 0 || TT - 1 < 0)
            { // reach edge
                deleteTree(curNode);
                return 0;
            }
...
void Postorder(tree *route)
{
    ...
    if (route != NULL)
    {
        ...
        if (route->Lchild != NULL && route->Rchild != NULL)
        {
            printf("%d %d %d %d %d %d %d\n", route->data_front, route->data_rear, route->Lchild->data_front, route->Lchild->data_rear, route->Rchild->data_front, route->Rchild->data_rear, (route->Time) + 1);
        }
    }
}
